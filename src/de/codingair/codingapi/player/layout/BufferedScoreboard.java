...
public class BufferedScoreboard {
	...
	public static BufferedScoreboard getBufferedScoreboard(Player p) {
		for(Player player : bufferedScoreboards.keySet()) {
			if(player.getName().equals(p.getName())) return bufferedScoreboards.get(player);
		}
		
		return new BufferedScoreboard(p, null, true);
	}
	...
	public static BufferedScoreboard destroyBoard(Player p) {
		...
		getBufferedScoreboard(toRemove).reset();
		return bufferedScoreboards.remove(toRemove);
	}
}
