package de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer;
...
import de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer.Extras.EquipmentType;
import de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer.Extras.FakePlayerDataWatcher;
import de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer.Extras.FakePlayerListener;
import de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer.Extras.Modules.*;
...
import de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer.Extras.Motions.FakePlayerMotionLook;
import de.CodingAir.v1_6.CodingAPI.CustomEntity.FakePlayer.Extras.Motions.FakePlayerMotionPosition;
...
public class FakePlayer implements Removable {
	...
	public static final double MOVE_SPEED = 4.3D / 20;
	public static final double SPRINT_SPEED = 6.5D / 20;
	public static final double MOVE_TICKS = 0.429D;
	public static final double JUMP_TICKS = 3.0D;
	public static final long RETRY_TIME = 3L;
	public static final long TABLIST_REMOVE_TIME = 20L;
	public static final int VIEW_RADIUS = 160;
	...
	public void updateHeadRotation(float yaw, float pitch) {
		...
		Object packet = IReflection.getConstructor(PacketUtils.PacketPlayOutEntityLookClass, new Class[]{int.class, byte.class, byte.class, boolean.class}).newInstance(this.getEntityId(), toAngle(yaw), toAngle(pitch), false);
		Object packetHead = IReflection.getConstructor(PacketUtils.PacketPlayOutEntityHeadRotationClass, new Class[]{PacketUtils.EntityClass, byte.class}).newInstance(this.player, toAngle(yaw));
	...
	public void spawn(boolean tablist, Player update) {
		...
		if(update == null) {
			...
			Runnable runnable = () -> {
				...
				if(!tablist) {
					Bukkit.getScheduler().runTaskLater(plugin, () -> sendPacket(update, tabRemove), TABLIST_REMOVE_TIME);
				}
			};
			...
			if(!isReady(update)) {
				new BukkitRunnable() {
					@Override
					public void run() {
						if(Bukkit.getPlayer(update.getUniqueId()) == null) this.cancel();
